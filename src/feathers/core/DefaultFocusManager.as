	...
	public class DefaultFocusManager implements IFocusManager
	{
		...
		public function set isEnabled(value:Boolean):void
		{
			...
			if (this._isEnabled)
			{
				...
				this._starling.nativeStage.addEventListener(
						FocusEvent.KEY_FOCUS_CHANGE,
						stage_keyFocusChangeHandler,
						false,
						0,
						true
				);
			...
			else
			{
				...
				this._starling.nativeStage.removeEventListener(
						FocusEvent.KEY_FOCUS_CHANGE,
						stage_keyFocusChangeHandler
				);
				this._starling.nativeStage.addEventListener(
		...
		public function set focus(value:IFocusDisplayObject):void
		{
			...
			if (this._isEnabled)
			{
				if (this._focus)
				{
					...
					nativeStage.focus.addEventListener(
							FocusEvent.FOCUS_OUT,
							nativeFocus_focusOutHandler,
							false,
							0,
							true
					);
		...
		protected function setFocusManager(target:DisplayObject):void
		{
			...
			if ((target is DisplayObjectContainer && !(target is IFocusDisplayObject)) || (target is IFocusContainer
																						   && IFocusContainer(target).isChildFocusEnabled))
			{
		...
		{
			if ((child is DisplayObjectContainer && !(child is IFocusDisplayObject)) || (child is IFocusContainer
																						 && IFocusContainer(child).isChildFocusEnabled))
			{
		...
		protected function stage_keyFocusChangeHandler(event:FocusEvent):void
		{
			...
			else if (event.shiftKey)
			{
				if (currentFocus)
				{
					...
					{
						newFocus =
								this.findPreviousContainerFocus(currentFocus.parent, DisplayObject(currentFocus), true);
					}
				}
				if (!newFocus)
				{
		...
		protected function topLevelContainer_touchHandler(event:TouchEvent):void
		{
			...
			do {
				if (target is IFocusDisplayObject)
				{
					...
					{
						if (!focusTarget || !(tempFocusTarget is IFocusContainer)
							|| !(IFocusContainer(tempFocusTarget).isChildFocusEnabled))
						{
