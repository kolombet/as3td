	...
	public class TimeLabel extends Label implements IMediaPlayerControl
	{
		...
		public function set mediaPlayer(value:IMediaPlayer):void
		{
			...
			if (this._mediaPlayer)
			{
				this._mediaPlayer.removeEventListener(
						MediaPlayerEventType.CURRENT_TIME_CHANGE,
						mediaPlayer_currentTimeChangeHandler
				);
				this._mediaPlayer.removeEventListener(
						MediaPlayerEventType.TOTAL_TIME_CHANGE,
						mediaPlayer_totalTimeChangeHandler
				);
			}
		...
		protected function updateText():void
		{
			...
			switch (displayMode)
			{
				...
				default:
				{
					this.text = this.secondsToTimeString(currentTime) + this._delimiter
								+ this.secondsToTimeString(totalTime);
				}
			}
		}
		...
		{
			if (!this._isEnabled || !this._toggleDisplayMode || !(this._displayMode === DISPLAY_MODE_CURRENT_TIME
				|| this._displayMode === DISPLAY_MODE_CURRENT_TIME))
			{
				this.touchPointID = -1;
				return;
			}
