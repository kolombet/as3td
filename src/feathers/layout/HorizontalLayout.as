	 ...
	 */ public class HorizontalLayout extends EventDispatcher implements IVariableVirtualLayout, ITrimmedVirtualLayout
	{
		...
		public function layout(items:Vector.<DisplayObject>, viewPortBounds:ViewPortBounds = null, result:LayoutBoundsResult = null):LayoutBoundsResult
		{
			...
			if (!this._useVirtualLayout || this._hasVariableItemDimensions || this._distributeWidths
				|| this._verticalAlign != VERTICAL_ALIGN_JUSTIFY || explicitHeight !== explicitHeight) //isNaN
			{
							...
							if (itemWidth != cachedWidth)
							{
									...
									itemWidth != calculatedTypicalItemWidth)
								{
									this.dispatchEventWith(
											Event.SCROLL,
											false,
											new Point(itemWidth - calculatedTypicalItemWidth, 0)
									);
								}
			...
			if (availableWidth !== availableWidth) //isNaN
			{
				...
				{
					availableWidth = (calculatedTypicalItemWidth + this._gap) * this._requestedColumnCount - this._gap
			...
			for (i = 0; i < discoveredItemCount; i++)
			{
				...
				else
				{
					if (layoutItem)
					{
						...
						if (layoutData)
						{
							...
							if (percentHeight === percentHeight) //!isNaN
							{
								...
								itemHeight = percentHeight * (availableHeight - this._paddingTop - this._paddingBottom)
											 / 100;
					...
					switch (this._verticalAlign)
					{
						...
						{
							item.y = item.pivotY + boundsY + this._paddingTop + Math.round(
											(verticalAlignHeight - this._paddingTop - this._paddingBottom - item.height)
											/ 2
									);
							break;
						}
		...
		public function measureViewPort(itemCount:int, viewPortBounds:ViewPortBounds = null, result:Point = null):Point
		{
			...
			if (needsWidth)
			{
				if (this._requestedColumnCount > 0)
				{
					var resultWidth:Number = (calculatedTypicalItemWidth + this._gap) * this._requestedColumnCount
											 - this._gap + this._paddingLeft + this._paddingRight
				}
				else
				{
		...
		public function getVisibleIndicesAtScrollPosition(scrollX:Number, scrollY:Number, width:Number, height:Number, itemCount:int, result:Vector.<int> = null):Vector.<int>
		{
			...
			if (!this._hasVariableItemDimensions)
			{
				...
				if (totalItemWidth < width)
				{
					...
					{
						indexOffset =
								Math.ceil(((width - totalItemWidth) / (calculatedTypicalItemWidth + this._gap)) / 2);
					}
				}
		...
		protected function calculateDistributedWidth(items:Vector.<DisplayObject>, explicitWidth:Number, minWidth:Number, maxWidth:Number):Number
		{
			...
			if (explicitWidth !== explicitWidth) //isNaN
			{
				...
				explicitWidth =
						maxItemWidth * itemCount + this._paddingLeft + this._paddingRight + this._gap * (itemCount - 1);
			...
			var availableSpace:Number = explicitWidth - this._paddingLeft - this._paddingRight - this._gap * (itemCount
																											  - 1);
