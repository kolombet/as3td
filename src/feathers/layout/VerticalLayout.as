	 ...
	 */ public class VerticalLayout extends EventDispatcher implements IVariableVirtualLayout, ITrimmedVirtualLayout, IGroupedLayout
	{
		...
		public function layout(items:Vector.<DisplayObject>, viewPortBounds:ViewPortBounds = null, result:LayoutBoundsResult = null):LayoutBoundsResult
		{
			...
			if (!this._useVirtualLayout || this._hasVariableItemDimensions || this._distributeHeights
				|| this._horizontalAlign != HORIZONTAL_ALIGN_JUSTIFY || explicitWidth !== explicitWidth) //isNaN
			{
							...
							if (itemHeight != cachedHeight)
							{
									...
									itemHeight != calculatedTypicalItemHeight)
								{
									this.dispatchEventWith(
											Event.SCROLL,
											false,
											new Point(0, itemHeight - calculatedTypicalItemHeight)
									);
								}
			...
			if (availableHeight !== availableHeight) //isNaN
			{
				...
				{
					availableHeight = this._requestedRowCount * (calculatedTypicalItemHeight + this._gap) - this._gap
				...
				else
				{
					if (layoutItem)
					{
						...
						if (layoutData)
						{
							...
							if (percentWidth === percentWidth) //!isNaN
							{
								...
								itemWidth =
										percentWidth * (availableWidth - this._paddingLeft - this._paddingRight) / 100;
					...
					switch (this._horizontalAlign)
					{
						...
						case HORIZONTAL_ALIGN_CENTER:
						{
							...
							//align in the center
							item.x = item.pivotX + boundsX + this._paddingLeft + Math.round(
											(horizontalAlignWidth - this._paddingLeft - this._paddingRight - item.width)
											/ 2
									);
							break;
						}
		...
		public function measureViewPort(itemCount:int, viewPortBounds:ViewPortBounds = null, result:Point = null):Point
		{
			...
			if (needsHeight)
			{
				if (this._requestedRowCount > 0)
				{
					var resultHeight:Number = (calculatedTypicalItemHeight + this._gap) * this._requestedRowCount
											  - this._gap;
				}
				else
				{
		...
		public function getVisibleIndicesAtScrollPosition(scrollX:Number, scrollY:Number, width:Number, height:Number, itemCount:int, result:Vector.<int> = null):Vector.<int>
		{
			...
			if (!this._hasVariableItemDimensions)
			{
				...
				if (totalItemHeight < height)
				{
					...
					{
						indexOffset =
								Math.ceil(((height - totalItemHeight) / (calculatedTypicalItemHeight + this._gap)) / 2);
					}
				}
		...
		protected function calculateDistributedHeight(items:Vector.<DisplayObject>, explicitHeight:Number, minHeight:Number, maxHeight:Number):Number
		{
			...
			if (explicitHeight !== explicitHeight) //isNaN
			{
				...
				explicitHeight =
						maxItemHeight * itemCount + this._paddingTop + this._paddingBottom + this._gap * (itemCount
																										  - 1);
			...
			var availableSpace:Number = explicitHeight - this._paddingTop - this._paddingBottom - this._gap * (itemCount
																											   - 1);
