		...
		public override function getBounds(targetSpace:DisplayObject, resultRect:Rectangle = null):Rectangle
		{
			...
			else
			{
				...
				MatrixUtil.transformCoords(
						HELPER_MATRIX,
						this._hitArea.x,
						this._hitArea.y + this._hitArea.height,
						HELPER_POINT
				);
				minX = minX < HELPER_POINT.x ? minX : HELPER_POINT.x;
				maxX = maxX > HELPER_POINT.x ? maxX : HELPER_POINT.x;
				...
				MatrixUtil.transformCoords(
						HELPER_MATRIX,
						this._hitArea.x + this._hitArea.width,
						this._hitArea.y,
						HELPER_POINT
				);
				minX = minX < HELPER_POINT.x ? minX : HELPER_POINT.x;
		...
		public function validate():void
		{
			...
			if (this._propertiesChanged || this._layoutChanged || this._renderingChanged)
			{
				...
				if (this._textures.direction == Scale3Textures.DIRECTION_VERTICAL)
				{
					...
					var scaledThirdRegionSize:Number = (this._frame.height - this._textures.firstRegionSize
														- this._textures.secondRegionSize) * oppositeEdgeScale;
				...
				else //horizontal
				{
					...
					scaledThirdRegionSize =
							(this._frame.width - this._textures.firstRegionSize - this._textures.secondRegionSize)
							* oppositeEdgeScale;
					sumFirstAndThird = scaledFirstRegionSize + scaledThirdRegionSize;
