...
package feathers.controls
{
	 ...
	 */ public class Slider extends FeathersControl implements IDirectionalScrollBar, IFocusDisplayObject
	{
		...
		protected function layoutThumb():void
		{
			...
			if (this._direction == DIRECTION_VERTICAL)
			{
				var trackScrollableHeight:Number = this.actualHeight - this.thumb.height - this._minimumPadding
												   - this._maximumPadding;
			...
			{
				var trackScrollableWidth:Number = this.actualWidth - this.thumb.width - this._minimumPadding
												  - this._maximumPadding;
		...
		protected function track_touchHandler(event:TouchEvent):void
		{
			...
			else
			{
				...
				if (this._direction == DIRECTION_VERTICAL)
				{
					...
					this._thumbStartY = Math.min(
							this.actualHeight - this.thumb.height,
							Math.max(0, HELPER_POINT.y - this.thumb.height / 2)
					);
				}
				else //horizontal
				{
					this._thumbStartX = Math.min(
							this.actualWidth - this.thumb.width,
