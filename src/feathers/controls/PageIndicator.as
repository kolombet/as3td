			...
			if (this.touchPointID >= 0)
			{
				...
				if (isInBounds)
				{
					...
					if (this._direction == DIRECTION_VERTICAL)
					{
						...
						{
							var symbolHeight:Number = this.selectedSymbol.height + (this.unselectedSymbols[0].height
																					+ this._gap) * lastPageIndex;
							var newIndex:int = Math.round(
									lastPageIndex * (HELPER_POINT.y - this.symbols[0].y) / symbolHeight
							);
							if (newIndex < 0)
							{
								newIndex = 0;
					...
					else
					{
						if (this._interactionMode == INTERACTION_MODE_PRECISE)
						{
							var symbolWidth:Number = this.selectedSymbol.width + (this.unselectedSymbols[0].width
																				  + this._gap) * lastPageIndex;
