...
package feathers.controls
{
	 ...
	 */ public class DateTimeSpinner extends FeathersControl
	{
		...
		{
			if (this.pendingScrollToDate && this.pendingScrollToDate.time === date.time && this.pendingScrollDuration
																						   === animationDuration)
			{
				return;
			}
		...
		protected function createYearList():void
		{
			...
			var listStyleName:String = (this._customListStyleName !== null) ? this._customListStyleName :
									   this.listStyleName;
		...
		protected function refreshValidRanges():void
		{
			...
			if (this._editingMode === EDITING_MODE_DATE_AND_TIME)
			{
				if (currentYear === this._minYear && currentMonth === this._minimum.month && currentDate
																							 === this._minimum.date)
				{
				...
				if (currentYear === this._maxYear && currentMonth === this._maximum.month && currentDate
																							 === this._maximum.date)
				{
				...
				if (currentYear === this._minYear && currentMonth === this._minimum.month && currentDate
																							 === this._minimum.date
					&& currentHours === this._minimum.hours)
				{
				...
				if (currentYear === this._maxYear && currentMonth === this._maximum.month && currentDate
																							 === this._maximum.date
					&& currentHours === this._maximum.hours)
				{
			...
			var dateAndTimeDatesCollection:ListCollection = this.dateAndTimeDatesList ?
															this.dateAndTimeDatesList.dataProvider : null;
			if (dateAndTimeDatesCollection && (oldMinYear !== this._minYear || oldMaxYear !== this._maxYear
			...
			if (hoursCollection && (oldMinHours !== this._minHours || oldMaxHours !== this._maxHours
									|| (this._showMeridiem && this._lastMeridiemValue
															  !== this.meridiemList.selectedIndex)))
			{
		...
		protected function useDefaultsIfNeeded():void
		{
			...
			else if (this._editingMode === EDITING_MODE_DATE_AND_TIME)
			{
				...
				this._minimum = new Date(
						this._listMinYear, MIN_MONTH_VALUE, MIN_DATE_VALUE, MIN_HOURS_VALUE, MIN_MINUTES_VALUE
				);
			}
			else
			{
			...
			else if (this._editingMode === EDITING_MODE_DATE_AND_TIME)
			{
				...
				this._maximum = new Date(
						this._listMaxYear,
						MAX_MONTH_VALUE,
						DAYS_IN_MONTH[MAX_MONTH_VALUE],
						MAX_HOURS_VALUE_24HOURS,
						MAX_MINUTES_VALUE
				);
			}
			else // date
		...
		protected function formatDateAndTimeWeekday(item:int):String
		{
			...
			if (this._todayLabel)
			{
				...
				if (HELPER_DATE.fullYear === this._lastValidate.fullYear && HELPER_DATE.month
																			=== this._lastValidate.month
		...
		protected function dateAndTimeDatesList_changeHandler(event:Event):void
		{
			...
			this._value.setFullYear(
					this._minimum.fullYear,
					this._minimum.month,
					this._minimum.date + this.dateAndTimeDatesList.selectedIndex
			);
			this.validateNewValue();
			this.refreshValidRanges();
			this.dispatchEventWith(Event.CHANGE);
