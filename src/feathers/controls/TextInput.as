...
package feathers.controls
{
	 ...
	 */ public class TextInput extends FeathersControl implements ITextBaselineControl, INativeFocusOwner, IStateContext
	{
		...
		protected function autoSizeIfNeeded():Boolean
		{
			...
			{
				newHeight =
						Math.max(this._originalSkinHeight, typicalTextHeight + this._paddingTop + this._paddingBottom);
			...
			var isMultiline:Boolean = this.textEditor is IMultilineTextEditor
									  && IMultilineTextEditor(this.textEditor).multiline;
		...
		protected function createTextEditor():void
		{
			...
			var factory:Function = this._textEditorFactory != null ? this._textEditorFactory :
								   FeathersControl.defaultTextEditorFactory;
			this.textEditor = ITextEditor(factory());
			var textEditorStyleName:String = this._customTextEditorStyleName != null ? this._customTextEditorStyleName :
											 this.textEditorStyleName;
		...
		protected function createPrompt():void
		{
			...
			var factory:Function = this._promptFactory != null ? this._promptFactory :
								   FeathersControl.defaultTextRendererFactory;
			this.promptTextRenderer = ITextRenderer(factory());
			var promptStyleName:String = this._customPromptStyleName != null ? this._customPromptStyleName :
										 this.promptStyleName;
		...
		protected function refreshBackgroundSkin():void
		{
			...
			else
			{
				this.currentBackground =
						DisplayObject(this._skinSelector.updateValue(this, this._currentState, this.currentBackground));
			}
		...
		protected function layoutChildren():void
		{
			...
			if (isMultiline)
			{
				...
				if (this.promptTextRenderer)
				{
					...
					this.promptTextRenderer.height =
							this.actualHeight - this.promptTextRenderer.y - this._paddingBottom;
				}
				if (this.currentIcon)
				{
			...
			else
			{
				switch (this._verticalAlign)
				{
					...
					{
						this.textEditor.y = this.actualHeight - this._paddingBottom - biggerHeight + biggerBaseline
											- this.textEditor.baseline;
						if (this.promptTextRenderer)
						{
							this.promptTextRenderer.y =
									this.actualHeight - this._paddingBottom - biggerHeight + biggerBaseline
									- promptBaseline;
						}
						if (this.currentIcon)
						{
					...
					{
						this.textEditor.y =
								biggerBaseline - this.textEditor.baseline + this._paddingTop + (this.actualHeight
																								- this._paddingTop
																								- this._paddingBottom
																								- biggerHeight) / 2;
						if (this.promptTextRenderer)
						{
							this.promptTextRenderer.y =
									biggerBaseline - promptBaseline + this._paddingTop + (this.actualHeight
																						  - this._paddingTop
																						  - this._paddingBottom
																						  - biggerHeight) / 2;
						}
						...
						{
							this.currentIcon.y =
									this._paddingTop + (this.actualHeight - this._paddingTop - this._paddingBottom
