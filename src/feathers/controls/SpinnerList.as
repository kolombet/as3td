...
package feathers.controls
{
	...
	public class SpinnerList extends List
	{
		...
		override protected function initialize():void
		{
			...
			{
				if (this._hasElasticEdges && this._verticalScrollPolicy == SCROLL_POLICY_AUTO
		...
		override protected function handlePendingScroll():void
		{
			if (this.pendingItemIndex >= 0)
			{
				...
				{
					this.pendingVerticalPageIndex = this.calculateNearestPageIndexForItem(
							itemIndex,
							this._verticalPageIndex,
							this._maxVerticalPageIndex
					);
				...
				{
					this.pendingHorizontalPageIndex = this.calculateNearestPageIndexForItem(
							itemIndex,
		...
		override protected function layoutChildren():void
		{
			...
			if (this._selectionOverlaySkin)
			{
				...
				if (this._maxVerticalPageIndex != this._minVerticalPageIndex)
				{
					this._selectionOverlaySkin.width =
							this.actualWidth - this._leftViewPortOffset - this._rightViewPortOffset;
					this._selectionOverlaySkin.height = this.actualPageHeight;
					this._selectionOverlaySkin.x = this._leftViewPortOffset;
					this._selectionOverlaySkin.y = Math.round(
							this._topViewPortOffset + (this.actualHeight - this._topViewPortOffset
													   - this._bottomViewPortOffset - this.actualPageHeight) / 2
					);
				}
				else if (this._maxHorizontalPageIndex != this._minHorizontalPageIndex)
				{
					this._selectionOverlaySkin.width = this.actualPageWidth;
					this._selectionOverlaySkin.height =
							this.actualHeight - this._topViewPortOffset - this._bottomViewPortOffset;
					this._selectionOverlaySkin.x = Math.round(
							this._leftViewPortOffset + (this.actualWidth - this._leftViewPortOffset
														- this._rightViewPortOffset - this.actualPageWidth) / 2
					);
		...
		protected function spinnerList_triggeredHandler(event:Event, item:Object):void
		{
			...
			if (this._maxVerticalPageIndex != this._minVerticalPageIndex)
			{
				itemIndex = this.calculateNearestPageIndexForItem(
						itemIndex,
						this._verticalPageIndex,
						this._maxVerticalPageIndex
				);
		...
		override protected function stage_keyDownHandler(event:KeyboardEvent):void
		{
			...
			if (changedSelection)
			{
				if (this._maxVerticalPageIndex != this._minVerticalPageIndex)
				{
					var pageIndex:int = this.calculateNearestPageIndexForItem(
							this._selectedIndex,
							this._verticalPageIndex,
							this._maxVerticalPageIndex
					);
				...
				{
					pageIndex = this.calculateNearestPageIndexForItem(
							this._selectedIndex,
							this._horizontalPageIndex,
							this._maxHorizontalPageIndex
					);
