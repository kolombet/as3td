...
package feathers.controls.supportClasses
{
	...
	public class GroupedListDataViewPort extends FeathersControl implements IViewPort
	{
		...
		{
			if (this.explicitVisibleWidth == value || (value !== value && this.explicitVisibleWidth
																		  !== this.explicitVisibleWidth)) //isNaN
			{
				return;
			}
		...
		{
			if (this.explicitVisibleHeight == value || (value !== value && this.explicitVisibleHeight
																		   !== this.explicitVisibleHeight)) //isNaN
			{
				return;
			}
		...
		public function set dataProvider(value:HierarchicalCollection):void
		{
			...
			if (this._dataProvider)
			{
				...
				this._dataProvider.removeEventListener(
						CollectionEventType.REPLACE_ITEM,
						dataProvider_replaceItemHandler
				);
		...
		public function getScrollPositionForIndex(groupIndex:int, itemIndex:int, result:Point = null):Point
		{
			...
			return this._layout.getScrollPositionForIndex(
					displayIndex, this._layoutItems, 0, 0, this.actualVisibleWidth, this.actualVisibleHeight, result
			);
		}
		...
		public function getNearestScrollPositionForIndex(groupIndex:int, itemIndex:int, result:Point = null):Point
		{
			...
			var displayIndex:int = this.locationToDisplayIndex(groupIndex, itemIndex);
			return this._layout.getNearestScrollPositionForIndex(
					displayIndex,
					...
					0,
					0,
					this.actualVisibleWidth,
					this.actualVisibleHeight,
					result
			);
		}
		...
		private function refreshLayoutTypicalItem():void
		{
			...
			if (typicalItem !== null)
			{
				...
				{
					typicalItemRenderer =
							this.createItemRenderer(typicalItem, 0, 0, 0, false, !newTypicalItemIsInDataProvider);
					if (!this._typicalItemIsInDataProvider && this._typicalItemRenderer)
					{
		...
		private function refreshSelection():void
		{
			for each(var item:DisplayObject in this._layoutItems)
			{
				...
				if (itemRenderer)
				{
					itemRenderer.isSelected =
							itemRenderer.groupIndex == this._selectedGroupIndex && itemRenderer.itemIndex
		...
		{
			var hasCustomFirstItemRenderer:Boolean = this._firstItemRendererType || this._firstItemRendererFactory
																					!= null
													 || this._customFirstItemRendererStyleName;
			...
			var hasCustomLastItemRenderer:Boolean = this._lastItemRendererType || this._lastItemRendererFactory != null
													|| this._customLastItemRendererStyleName;
			...
			var hasCustomSingleItemRenderer:Boolean = this._singleItemRendererType || this._singleItemRendererFactory
																					  != null
													  || this._customSingleItemRendererStyleName;
		...
		private function refreshRenderers():void
		{
			if (this._typicalItemRenderer)
			{
				if (this._typicalItemIsInDataProvider)
				{
					var itemStorage:ItemRendererFactoryStorage = this.factoryIDToStorage(
							this._typicalItemRenderer.factoryID,
							this._typicalItemRenderer.groupIndex,
							this._typicalItemRenderer.itemIndex
					);
		...
		private function findUnrenderedData():void
		{
			...
			if (useVirtualLayout)
			{
				...
				virtualLayout.getVisibleIndicesAtScrollPosition(
						this._horizontalScrollPosition,
						this._verticalScrollPosition,
						viewPortWidth,
						viewPortHeight,
				...
				if (this._typicalItemRenderer)
				{
					...
					this._minimumFirstAndLastItemCount = this._minimumSingleItemCount = this._minimumHeaderCount =
							this._minimumFooterCount =
									Math.ceil(maximumViewPortEdge / (minimumTypicalItemEdge * averageItemsPerGroup));
			...
			for (i = 0; i < groupCount; i++)
			{
				...
				for (var j:int = 0; j < currentItemCount; j++)
				{
					...
					{
						if (this._typicalItemRenderer && this._typicalItemIsInDataProvider
							&& this._typicalItemRenderer.groupIndex === i && this._typicalItemRenderer.itemIndex === j)
						{
							//the indices may have changed if items were added, removed,
							//or reordered in the data provider
		...
		private function findRendererForItem(item:Object, groupIndex:int, itemIndex:int, layoutIndex:int):void
		{
			...
			if (itemRenderer)
			{
				...
				{
					var storage:ItemRendererFactoryStorage = this.factoryIDToStorage(
							itemRenderer.factoryID, itemRenderer.groupIndex, itemRenderer.itemIndex
					);
		...
		private function renderUnrenderedData():void
		{
			...
			for (i = 0; i < rendererCount; i += 2)
			{
				...
				var headerRenderer:IGroupedListHeaderRenderer = this.createHeaderRenderer(
						item,
						groupIndex,
						layoutIndex,
						false
			...
			for (i = 0; i < rendererCount; i += 2)
			{
				...
				var footerRenderer:IGroupedListFooterRenderer = this.createFooterRenderer(
						item,
						groupIndex,
						layoutIndex,
						false
