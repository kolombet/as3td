	...
	public class ListDataViewPort extends FeathersControl implements IViewPort
	{
		...
		public function getScrollPositionForIndex(index:int, result:Point = null):Point
		{
			...
			return this._layout.getScrollPositionForIndex(
					index, this._layoutItems, 0, 0, this.actualVisibleWidth, this.actualVisibleHeight, result
			);
		}
			...
			if (typicalItem !== null)
			{
				...
				if (!typicalRenderer && this._typicalItemRenderer)
				{
					...
					var oldTypicalItemRemoved:Boolean = this._typicalItemIsInDataProvider
														&& this._dataProvider.getItemIndex(this._typicalItemRenderer.data)
														   < 0;
				...
				if (!typicalRenderer)
				{
					...
					//create a new one.
					typicalRenderer =
							this.createRenderer(typicalItem, typicalItemIndex, false, !newTypicalItemIsInDataProvider);
					if (!this._typicalItemIsInDataProvider && this._typicalItemRenderer)
					{
		...
		private function findUnrenderedData():void
		{
			...
			var canUseBeforeAndAfter:Boolean = this._layout is ITrimmedVirtualLayout && useVirtualLayout
											   && (!(this._layout is IVariableVirtualLayout)
												   || !IVariableVirtualLayout(this._layout).hasVariableItemDimensions)
