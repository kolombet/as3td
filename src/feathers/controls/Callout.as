...
package feathers.controls
{
	 ...
	 */ public class Callout extends FeathersControl
	{
		...
		protected static function positionBestSideOfOrigin(callout:Callout, globalOrigin:Rectangle):void
		{
			...
			var downSpace:Number = (Starling.current.stage.stageHeight - HELPER_POINT.y) - (globalOrigin.y
																							+ globalOrigin.height);
			...
			var rightSpace:Number = (Starling.current.stage.stageWidth - HELPER_POINT.x) - (globalOrigin.x
																							+ globalOrigin.width);
		...
		protected static function positionBelowOrigin(callout:Callout, globalOrigin:Rectangle):void
		{
			...
			var xPosition:Number = Math.max(
					stagePaddingLeft,
					Math.min(Starling.current.stage.stageWidth - HELPER_POINT.x - stagePaddingRight, idealXPosition)
			);
			callout.x = xPosition;
		...
		protected static function positionToRightOfOrigin(callout:Callout, globalOrigin:Rectangle):void
		{
			...
			var yPosition:Number = Math.max(
					stagePaddingTop,
					Math.min(Starling.current.stage.stageHeight - HELPER_POINT.y - stagePaddingBottom, idealYPosition)
			);
			callout.y = yPosition;
			if (callout._isValidating)
			{
		...
		public function set content(value:DisplayObject):void
		{
			...
			if (this._content)
			{
				...
				{
					IFeathersControl(this._content).removeEventListener(
							FeathersEventType.RESIZE,
							content_resizeHandler
					);
				}
		...
		protected function measureWithArrowPosition(arrowPosition:String, result:Point = null):Point
		{
			...
			{
				var minWidthWithBottomArrow:Number = this._bottomArrowSkin.width + this._paddingLeft
													 + this._paddingRight;
			...
			{
				var minHeightWithRightArrow:Number = this._rightArrowSkin.height + this._paddingTop
													 + this._paddingBottom;
			...
			if (this._content is IFeathersControl)
			{
				...
				var minContentHeight:Number = minHeight - topOrBottomArrowHeight - this._paddingTop
											  - this._paddingBottom;
				var maxContentHeight:Number = maxHeight - topOrBottomArrowHeight - this._paddingTop
											  - this._paddingBottom;
		...
		{
			var xPosition:Number = (this._leftArrowSkin && this._arrowPosition == ARROW_POSITION_LEFT) ?
								   this._leftArrowSkin.width + this._leftArrowGap : 0;
			var yPosition:Number = (this._topArrowSkin && this._arrowPosition == ARROW_POSITION_TOP) ?
								   this._topArrowSkin.height + this._topArrowGap : 0;
			var widthOffset:Number = (this._rightArrowSkin && this._arrowPosition == ARROW_POSITION_RIGHT) ?
									 this._rightArrowSkin.width + this._rightArrowGap : 0;
			var heightOffset:Number = (this._bottomArrowSkin && this._arrowPosition == ARROW_POSITION_BOTTOM) ?
									  this._bottomArrowSkin.height + this._bottomArrowGap : 0;
			...
			if (this.currentArrowSkin)
			{
				if (this._arrowPosition == ARROW_POSITION_LEFT)
				{
					...
					this._leftArrowSkin.y = this._arrowOffset + yPosition + Math.round(
									(backgroundHeight - this._leftArrowSkin.height) / 2
							);
					this._leftArrowSkin.y = Math.min(
							yPosition + backgroundHeight - this._paddingBottom - this._leftArrowSkin.height,
							Math.max(yPosition + this._paddingTop, this._leftArrowSkin.y)
					);
				}
				else if (this._arrowPosition == ARROW_POSITION_RIGHT)
				{
					this._rightArrowSkin.x = xPosition + backgroundWidth + this._rightArrowGap;
					this._rightArrowSkin.y = this._arrowOffset + yPosition + Math.round(
									(backgroundHeight - this._rightArrowSkin.height) / 2
							);
					this._rightArrowSkin.y = Math.min(
							yPosition + backgroundHeight - this._paddingBottom - this._rightArrowSkin.height,
							Math.max(yPosition + this._paddingTop, this._rightArrowSkin.y)
					);
				}
				else if (this._arrowPosition == ARROW_POSITION_BOTTOM)
				{
					this._bottomArrowSkin.x = this._arrowOffset + xPosition + Math.round(
									(backgroundWidth - this._bottomArrowSkin.width) / 2
							);
					this._bottomArrowSkin.x = Math.min(
							xPosition + backgroundWidth - this._paddingRight - this._bottomArrowSkin.width,
							Math.max(xPosition + this._paddingLeft, this._bottomArrowSkin.x)
					);
					this._bottomArrowSkin.y = yPosition + backgroundHeight + this._bottomArrowGap;
				}
				else //top
				{
					this._topArrowSkin.x = this._arrowOffset + xPosition + Math.round(
									(backgroundWidth - this._topArrowSkin.width) / 2
							);
					this._topArrowSkin.x = Math.min(
							xPosition + backgroundWidth - this._paddingRight - this._topArrowSkin.width,
							Math.max(xPosition + this._paddingLeft, this._topArrowSkin.x)
					);
		...
		protected function callout_addedToStageHandler(event:Event):void
		{
			...
			Starling.current.nativeStage.addEventListener(
					...
					callout_nativeStage_keyDownHandler,
					false,
					priority,
					true
			);
		...
		protected function callout_removedFromStageHandler(event:Event):void
		{
			...
			Starling.current.nativeStage.removeEventListener(
					KeyboardEvent.KEY_DOWN,
					callout_nativeStage_keyDownHandler
			);
		}
		...
		protected function stage_touchHandler(event:TouchEvent):void
		{
			...
			if (!this._isReadyToClose || (!this.closeOnTouchEndedOutside && !this.closeOnTouchBeganOutside)
				|| this.contains(target) || (PopUpManager.isPopUp(this) && !PopUpManager.isTopLevelPopUp(this)))
			{
				return;
			}
			
			if (this._origin == target || (this._origin is DisplayObjectContainer
										   && DisplayObjectContainer(this._origin).contains(target)))
			{
				return;
			}
