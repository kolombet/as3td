	 ...
	 */ public class ToggleSwitch extends FeathersControl implements IToggle, IFocusDisplayObject, ITextBaselineControl
	{
		...
		protected function autoSizeIfNeeded():Boolean
		{
			...
			if (needsWidth)
			{
				...
				{
					newWidth =
							Math.min(this.onTrackSkinOriginalWidth, this.offTrackSkinOriginalWidth) + this.thumb.width
																									  / 2;
				}
				else
				{
		...
		protected function createOnTrack():void
		{
			...
			var onTrackStyleName:String = this._customOnTrackStyleName != null ? this._customOnTrackStyleName :
										  this.onTrackStyleName;
		...
		protected function createOffTrack():void
		{
			if (this._trackLayoutMode == TRACK_LAYOUT_MODE_ON_OFF)
			{
				...
				var offTrackStyleName:String = this._customOffTrackStyleName != null ? this._customOffTrackStyleName :
											   this.offTrackStyleName;
		...
		protected function createLabels():void
		{
			...
			var offLabelStyleName:String = this._customOffLabelStyleName != null ? this._customOffLabelStyleName :
										   this.offLabelStyleName;
			...
			var onLabelStyleName:String = this._customOnLabelStyleName != null ? this._customOnLabelStyleName :
										  this.onLabelStyleName;
		...
		protected function layoutChildren():void
		{
			...
			var maxLabelWidth:Number = Math.max(
					0,
					this.actualWidth - this.thumb.width - this._paddingLeft - this._paddingRight
			);
		...
		protected function thumb_touchHandler(event:TouchEvent):void
		{
			...
			if (this._touchPointID >= 0)
			{
				...
				var trackScrollableWidth:Number = this.actualWidth - this._paddingLeft - this._paddingRight
												  - this.thumb.width;
				if (touch.phase == TouchPhase.MOVED)
				{
					var xOffset:Number = HELPER_POINT.x - this._touchStartX;
					var xPosition:Number = Math.min(
							Math.max(this._paddingLeft, this._thumbStartX + xOffset),
							this._paddingLeft + trackScrollableWidth
					);
					this.thumb.x = xPosition;
					this.layoutTracks();
				}
