	 ...
	 */ public class Button extends FeathersControl implements IFocusDisplayObject, IStateContext, ITextBaselineControl
	{
		...
		public function set downLabelProperties(value:Object):void
		{
			...
			var oldValue:PropertyProxy = PropertyProxy(
					this._labelPropertiesSelector.getValueForState(
							STATE_DOWN,
							false
					)
			);
		...
		{
			var value:PropertyProxy = PropertyProxy(
					this._labelPropertiesSelector.getValueForState(
							STATE_DISABLED,
							false
					)
		...
		override public function render(support:RenderSupport, parentAlpha:Number):void
		{
			...
			if (scale !== 1)
			{
				support.translateMatrix(
						Math.round((1 - scale) / 2 * this.actualWidth), Math.round((1 - scale) / 2 * this.actualHeight)
				);
		...
		protected function createLabel():void
		{
			...
			{
				var factory:Function = this._labelFactory != null ? this._labelFactory :
									   FeathersControl.defaultTextRendererFactory;
				this.labelTextRenderer = ITextRenderer(factory());
				var labelStyleName:String = this._customLabelStyleName != null ? this._customLabelStyleName :
											this.labelStyleName;
		...
		protected function refreshIcon():void
		{
			...
			{
				this.currentIcon =
						DisplayObject(this._iconSelector.updateValue(this, this._currentState, this.currentIcon));
			}
			...
			if (this.currentIcon != oldIcon)
			{
				if (oldIcon)
				{
					...
					{
						IFeathersControl(oldIcon).removeEventListener(
								FeathersEventType.RESIZE,
								currentIcon_resizeHandler
						);
					}
				...
				if (this.currentIcon)
				{
					...
					if (this.currentIcon is IFeathersControl)
					{
						IFeathersControl(this.currentIcon).addEventListener(
								FeathersEventType.RESIZE,
								currentIcon_resizeHandler
						);
					}
		...
		protected function layoutContent():void
		{
			...
			else if ((!this._label || !this.labelTextRenderer) && this.currentIcon && this._iconPosition
																					  != ICON_POSITION_MANUAL)
			{
			...
			if (this._label && this.labelTextRenderer)
			{
				...
				if (this.currentIcon)
				{
					...
					if (this._iconPosition == ICON_POSITION_LEFT || this._iconPosition == ICON_POSITION_LEFT_BASELINE
						|| this._iconPosition == ICON_POSITION_RIGHT || this._iconPosition
		...
		protected function positionSingleChild(displayObject:DisplayObject):void
		{
			...
			{
				displayObject.x = this._paddingLeft + Math.round(
								(this.actualWidth - this._paddingLeft - this._paddingRight - displayObject.width) / 2
						);
			}
			...
			{
				displayObject.y = this._paddingTop + Math.round(
								(this.actualHeight - this._paddingTop - this._paddingBottom - displayObject.height) / 2
						);
			}
		}
		...
		protected function positionLabelAndIcon():void
		{
			...
			if (this._iconPosition == ICON_POSITION_LEFT || this._iconPosition == ICON_POSITION_RIGHT)
			{
				...
				{
					this.currentIcon.y = this._paddingTop + Math.round(
									(this.actualHeight - this._paddingTop - this._paddingBottom
									 - this.currentIcon.height) / 2
							);
				}
			}
			else if (this._iconPosition == ICON_POSITION_LEFT_BASELINE || this._iconPosition
																		  == ICON_POSITION_RIGHT_BASELINE)
			{
				this.currentIcon.y =
						this.labelTextRenderer.y + (this.labelTextRenderer.baseline) - this.currentIcon.height;
			}
			else //top or bottom
			{
				if (this._horizontalAlign == HORIZONTAL_ALIGN_LEFT)
				...
				{
					this.currentIcon.x = this._paddingLeft + Math.round(
									(this.actualWidth - this._paddingLeft - this._paddingRight - this.currentIcon.width)
									/ 2
							);
				}
			}
