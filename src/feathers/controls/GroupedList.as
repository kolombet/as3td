	 ...
	 */ public class GroupedList extends Scroller implements IFocusContainer
	{
		...
		override public function get isFocusEnabled():Boolean
		{
			return (this._isSelectable || this._minHorizontalScrollPosition != this._maxHorizontalScrollPosition
					|| this._minVerticalScrollPosition != this._maxVerticalScrollPosition) && this._isEnabled
				   && this._isFocusEnabled;
		}
		...
		public function scrollToDisplayIndex(groupIndex:int, itemIndex:int = -1, animationDuration:Number = 0):void
		{
			...
			if (this.pendingGroupIndex == groupIndex && this.pendingItemIndex == itemIndex && this.pendingScrollDuration
																							  == animationDuration)
			{
				return;
			}
		...
		override protected function handlePendingScroll():void
		{
			if (this.pendingGroupIndex >= 0)
			{
				...
				{
					var pendingData:Object = this._dataProvider.getItemAt(
							this.pendingGroupIndex,
							this.pendingItemIndex
					);
				}
				else
				{
				...
				{
					this.dataViewPort.getScrollPositionForIndex(
							this.pendingGroupIndex,
							this.pendingItemIndex,
							HELPER_POINT
					);
		...
		override protected function stage_keyDownHandler(event:KeyboardEvent):void
		{
			...
			if (changedSelection)
			{
				this.dataViewPort.getNearestScrollPositionForIndex(
		...
		private function layout_scrollHandler(event:Event, scrollOffset:Point):void
		{
			...
			if (this._horizontalAutoScrollTween)
			{
				...
				this.throwTo(
						...
						NaN,
						this._horizontalAutoScrollTween.totalTime - this._horizontalAutoScrollTween.currentTime
				);
			}
			...
			if (this._verticalAutoScrollTween)
			{
				...
				this.throwTo(
						NaN,
						this._targetVerticalScrollPosition,
						this._verticalAutoScrollTween.totalTime - this._verticalAutoScrollTween.currentTime
				);
			}
		}
	}
