	 ...
	 */ public class Drawers extends FeathersControl
	{
			...
			if (this._content)
			{
				if (this._contentEventDispatcherChangeEventType)
				{
					this._content.removeEventListener(
							this._contentEventDispatcherChangeEventType,
							content_eventDispatcherChangeHandler
					);
				}
		...
		public function set overlaySkin(value:DisplayObject):void
		{
			...
			if (this._overlaySkin)
			{
				...
				this._overlaySkin.visible = this.isTopDrawerOpen || this.isRightDrawerOpen || this.isBottomDrawerOpen
											|| this.isLeftDrawerOpen;
		...
		public function set topDrawerToggleEventType(value:String):void
		{
			...
			{
				this.contentEventDispatcher.removeEventListener(
						this._topDrawerToggleEventType,
						content_topDrawerToggleEventTypeHandler
				);
			}
			this._topDrawerToggleEventType = value;
			if (this.contentEventDispatcher && this._topDrawerToggleEventType)
			{
				this.contentEventDispatcher.addEventListener(
						this._topDrawerToggleEventType,
						content_topDrawerToggleEventTypeHandler
				);
			}
		...
		public function set rightDrawerToggleEventType(value:String):void
		{
			...
			{
				this.contentEventDispatcher.removeEventListener(
						this._rightDrawerToggleEventType,
						content_rightDrawerToggleEventTypeHandler
				);
			}
		...
		public function set bottomDrawerToggleEventType(value:String):void
		{
			...
			{
				this.contentEventDispatcher.removeEventListener(
						this._bottomDrawerToggleEventType,
						content_bottomDrawerToggleEventTypeHandler
				);
			}
		...
		public function set leftDrawerToggleEventType(value:String):void
		{
			...
			{
				this.contentEventDispatcher.removeEventListener(
						this._leftDrawerToggleEventType,
						content_leftDrawerToggleEventTypeHandler
				);
			}
		...
		override public function hitTest(localPoint:Point, forTouch:Boolean = false):DisplayObject
		{
			...
			if (result)
			{
				...
				if (this.isTopDrawerOpen && result != this._topDrawer && !(this._topDrawer is DisplayObjectContainer
					&& DisplayObjectContainer(this._topDrawer).contains(result)))
				{
					return this;
				}
				else if (this.isRightDrawerOpen && result != this._rightDrawer && !(this._rightDrawer
																					is DisplayObjectContainer
						 && DisplayObjectContainer(this._rightDrawer).contains(result)))
				{
					return this;
				}
				else if (this.isBottomDrawerOpen && result != this._bottomDrawer && !(this._bottomDrawer
																					  is DisplayObjectContainer
						 && DisplayObjectContainer(this._bottomDrawer).contains(result)))
				{
					return this;
				}
				else if (this.isLeftDrawerOpen && result != this._leftDrawer && !(this._leftDrawer
																				  is DisplayObjectContainer
						 && DisplayObjectContainer(this._leftDrawer).contains(result)))
				{
					return this;
				}
				return result;
		...
		protected function contentToContentEventDispatcher():EventDispatcher
		{
			...
			else if (this._contentEventDispatcherField != null && this._content
					 && this._content.hasOwnProperty(this._contentEventDispatcherField))
			{
		...
		protected function refreshOverlayState():void
		{
			...
			var showOverlay:Boolean = this._isTopDrawerOpen || this._isRightDrawerOpen || this._isBottomDrawerOpen
			...
			if (!this.isTopDrawerOpen && !this.isRightDrawerOpen && !this.isBottomDrawerOpen && !this.isLeftDrawerOpen)
			{
				...
				if (this._openGesture == OPEN_GESTURE_DRAG_CONTENT_EDGE)
				{
					...
					if (!isNearAnyEdge)
					{
						if (this._rightDrawer && !this.isRightDrawerDocked)
						{
							var rightInches:Number = (this.actualWidth - localX) / (DeviceCapabilities.dpi
						...
						if (!isNearAnyEdge)
						{
							if (this._bottomDrawer && !this.isBottomDrawerDocked)
							{
								var bottomInches:Number = (this.actualHeight - localY) / (DeviceCapabilities.dpi
														  / Starling.contentScaleFactor);
							...
							if (!isNearAnyEdge)
							{
								if (this._leftDrawer && !this.isLeftDrawerDocked)
								{
									var leftInches:Number = localX / (DeviceCapabilities.dpi
															/ Starling.contentScaleFactor);
		...
		protected function handleDragEnd():void
		{
			...
			var inchesPerSecondX:Number = 1000 * (sum / totalWeight) / (DeviceCapabilities.dpi
										  / Starling.contentScaleFactor);
			...
			var inchesPerSecondY:Number = 1000 * (sum / totalWeight) / (DeviceCapabilities.dpi
										  / Starling.contentScaleFactor);
			...
			else if (this._isDraggingRightDrawer)
			{
				...
				else if (this._openMode === OPEN_MODE_ABOVE)
				{
					this._isRightDrawerOpen =
							roundToNearest(this.actualWidth - this._rightDrawer.x, this._rightDrawer.width) != 0;
				}
				else //bottom
				{
			...
			else if (this._isDraggingBottomDrawer)
			{
				...
				else if (this._openMode === OPEN_MODE_ABOVE)
				{
					this._isBottomDrawerOpen =
							roundToNearest(this.actualHeight - this._bottomDrawer.y, this._bottomDrawer.height) != 0;
				}
				else //below
				{
		...
		{
			var horizontalInchesMoved:Number = (this._currentTouchX - this._startTouchX) / (DeviceCapabilities.dpi
											   / Starling.contentScaleFactor);
			var verticalInchesMoved:Number = (this._currentTouchY - this._startTouchY) / (DeviceCapabilities.dpi
											 / Starling.contentScaleFactor);
		...
		protected function checkForDragToOpen():void
		{
			...
			else if (this._rightDrawer && !this.isRightDrawerDocked && horizontalInchesMoved
			...
			else if (this._bottomDrawer && !this.isBottomDrawerDocked && verticalInchesMoved
																		 <= -this._minimumDragDistance)
			{
				this._isDragging = true;
				this._isDraggingBottomDrawer = true;
		...
		protected function drawers_addedToStageHandler(event:Event):void
		{
			...
			Starling.current.nativeStage.addEventListener(
					...
					drawers_nativeStage_keyDownHandler,
					false,
					priority,
					true
			);
		...
		protected function drawers_touchHandler(event:TouchEvent):void
		{
			...
			if (this.touchPointID >= 0)
			{
				...
				if (touch.phase == TouchPhase.MOVED)
				{
					...
					if (!this._isDragging)
					{
						if (this.isTopDrawerOpen || this.isRightDrawerOpen || this.isBottomDrawerOpen
							|| this.isLeftDrawerOpen)
						{
				...
				else if (touch.phase == TouchPhase.ENDED)
				{
					...
					{
						ExclusiveTouch.forStage(this.stage).removeEventListener(
								Event.CHANGE,
								exclusiveTouch_changeHandler
						);
						if (this.isTopDrawerOpen || this.isRightDrawerOpen || this.isBottomDrawerOpen
							|| this.isLeftDrawerOpen)
						{
