...
package feathers.controls
{
	 ...
	 */ public class ImageLoader extends FeathersControl
	{
		...
		public function set textureQueueDuration(value:Number):void
		{
			...
			if (this._delayTextureCreation)
			{
				if ((this._pendingBitmapDataTexture || this._pendingRawTextureData) && oldDuration
																					   == Number.POSITIVE_INFINITY
					&& this._textureQueueDuration < Number.POSITIVE_INFINITY)
				{
		...
		override public function render(support:RenderSupport, parentAlpha:Number):void
		{
			if (this._snapToPixels)
			{
				...
				support.translateMatrix(
						Math.round(HELPER_MATRIX.tx) - HELPER_MATRIX.tx,
						Math.round(HELPER_MATRIX.ty) - HELPER_MATRIX.ty
				);
			}
				...
				else if (sourceURL != this._lastURL)
				{
					...
					if (isATFURL(sourceURL))
					{
						...
						this.urlLoader.addEventListener(
								SecurityErrorEvent.SECURITY_ERROR,
								rawDataLoader_securityErrorHandler
						);
		...
		protected function layout():void
		{
			...
			else
			{
				...
				{
					this.image.x = this._paddingLeft + ((this.actualWidth - this._paddingLeft - this._paddingRight)
														- imageWidth) / 2;
				}
				else //left
				{
				...
				{
					this.image.y = this._paddingTop + ((this.actualHeight - this._paddingTop - this._paddingBottom)
													   - imageHeight) / 2;
				}
				else //top
				{
		...
		protected function replaceBitmapDataTexture(bitmapData:BitmapData):void
		{
			...
			if (!this._texture)
			{
				...
				this._texture = Texture.empty(
						bitmapData.width / this._scaleFactor,
						bitmapData.height / this._scaleFactor,
						true,
						false,
						false,
						this._scaleFactor,
				...
				var sourceURL:String = this._source as String;
				this._texture.root.onRestore = this.createTextureOnRestore(
						this._texture, sourceURL, this._textureFormat, this._scaleFactor
				);
		...
		protected function loader_completeHandler(event:flash.events.Event):void
		{
			...
			//create a new one.
			var canReuseTexture:Boolean = this._texture && this._texture.nativeWidth === bitmapData.width
										  && this._texture.nativeHeight === bitmapData.height && this._texture.scale
																								 === this._scaleFactor
