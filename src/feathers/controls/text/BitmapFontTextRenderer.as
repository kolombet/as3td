	...
	public class BitmapFontTextRenderer extends FeathersControl implements ITextRenderer, IStateObserver
	{
		...
		public function measureText(result:Point = null):Point
		{
			...
			for (var i:int = 0; i < charCount; i++)
			{
				...
				if (this._wordWrap)
				{
					...
					var previousCharIsWhitespace:Boolean = previousCharID == CHARACTER_ID_SPACE || previousCharID
																								   == CHARACTER_ID_TAB;
		...
		protected function layoutCharacters(result:Point = null):Point
		{
			...
			for (var i:int = 0; i < charCount; i++)
			{
				...
				if (this._wordWrap)
				{
																			...
																			> FUZZY_MAX_WIDTH_PADDING)
					{
						if (isAligned)
						{
							...
							this.alignBuffer(
									maxLineWidth,
									startXOfPreviousWord - widthOfWhitespaceAfterWord,
									wordLength
							);
				...
				if (this._wordWrap || isAligned)
				{
					var charLocation:CharLocation = CHAR_LOCATION_POOL.length > 0 ? CHAR_LOCATION_POOL.shift() :
													new CharLocation();
		...
		protected function getTruncatedText(width:Number):String
		{
			...
			if (width == Number.POSITIVE_INFINITY || this._wordWrap
