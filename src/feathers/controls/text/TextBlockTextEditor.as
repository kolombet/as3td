...
package feathers.controls.text
{
	 ...
	 */ public class TextBlockTextEditor extends TextBlockTextRenderer implements ITextEditor, INativeFocusOwner
	{
		...
		override protected function initialize():void
		{
			...
			this._nativeFocus.addEventListener(
					flash.events.Event.SELECT_ALL,
					nativeFocus_selectAllHandler,
					false,
					0,
					true
			);
		...
		{
			var showCursor:Boolean = this._selectionBeginIndex >= 0 && this._selectionBeginIndex
		...
		protected function getCursorIndexFromSelectionRange():int
		{
			...
			if (this.touchPointID >= 0 && this._selectionAnchorIndex >= 0 && this._selectionAnchorIndex
																			 == this._selectionEndIndex)
			{
		...
		protected function deleteSelectedText():void
		{
			...
			this.text =
					currentValue.substr(0, this._selectionBeginIndex) + currentValue.substr(this._selectionEndIndex);
			this.validate();
		...
		protected function replaceSelectedText(text:String):void
		{
			...
			var newText:String = currentValue.substr(0, this._selectionBeginIndex) + text
								 + currentValue.substr(this._selectionEndIndex);
		...
		protected function textEditor_touchHandler(event:TouchEvent):void
		{
			...
			if (this.touchPointID >= 0)
			{
				...
				this.selectRange(
						this._selectionAnchorIndex,
						this.getSelectionIndexAtPoint(HELPER_POINT.x, HELPER_POINT.y)
				);
		...
		{
			if (!this._isEnabled || (!this._isEditable && !this._isSelectable) || this.touchPointID >= 0
			...
			else if (event.keyCode == Keyboard.LEFT)
			{
				...
				else
				{
					...
					{
						newIndex =
								TextInputNavigation.findPreviousWordStartIndex(this._text, this._selectionBeginIndex);
					}
					else
					{
						newIndex = this._selectionBeginIndex - 1;
			...
			if (newIndex < 0)
			{
				...
				if (event.keyCode == Keyboard.DELETE)
				{
					if (event.altKey || event.ctrlKey)
					{
						var nextWordStartIndex:int = TextInputNavigation.findNextWordStartIndex(
								this._text,
								this._selectionEndIndex
						);
						this.text = currentValue.substr(0, this._selectionBeginIndex)
									+ currentValue.substr(nextWordStartIndex);
					}
					else if (this._selectionBeginIndex != this._selectionEndIndex)
					{
					...
					{
						this.text = currentValue.substr(0, this._selectionBeginIndex)
									+ currentValue.substr(this._selectionEndIndex + 1);
					}
				}
				else if (event.keyCode == Keyboard.BACKSPACE)
				{
					...
					{
						newIndex = this._selectionBeginIndex - 1;
						this.text = currentValue.substr(0, this._selectionBeginIndex - 1)
									+ currentValue.substr(this._selectionEndIndex);
					}
				}
			}
		...
		{
			if (!this._isEnabled || (!this._isEditable && !this._isSelectable) || this._selectionBeginIndex
																				  == this._selectionEndIndex
				|| this._displayAsPassword)
			{
				return;
			}
