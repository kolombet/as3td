...
package feathers.controls.text
{
	 ...
	 */ public class TextFieldTextEditor extends FeathersControl implements ITextEditor, INativeFocusOwner, IStateObserver
	{
		...
		override public function dispose():void
		{
			...
			if (this.textField)
			{
				...
				this.textField.removeEventListener(
						...
						textField_softKeyboardActivateHandler
				);
				this.textField.removeEventListener(
						SoftKeyboardEvent.SOFT_KEYBOARD_DEACTIVATE,
						textField_softKeyboardDeactivateHandler
				);
			}
				...
				if (position)
				{
					...
					else
					{
						...
						if (this._pendingSelectionBeginIndex < 0)
						{
							if (this._multiline)
							{
								var lineIndex:int = int(positionY / this.textField.getLineMetrics(0).height)
													+ (this.textField.scrollV - 1);
								try
								{
									this._pendingSelectionBeginIndex = this.textField.getLineOffset(lineIndex)
																	   + this.textField.getLineLength(lineIndex);
							...
							{
								this._pendingSelectionBeginIndex = this.getSelectionIndexAtPoint(
										positionX,
										this.textField.getLineMetrics(0).ascent / 2
								);
								if (this._pendingSelectionBeginIndex < 0)
								{
									this._pendingSelectionBeginIndex = this._text.length;
		...
		protected function layout(sizeInvalid:Boolean):void
		{
			...
			if (!this._textFieldHasFocus && (sizeInvalid || stylesInvalid || dataInvalid || stateInvalid
		...
		{
			if (this.textSnapshot && this.textSnapshot.texture && this.textSnapshot.texture.scale
		...
		protected function refreshSnapshot():void
		{
			...
			HELPER_MATRIX.translate(
					this._textFieldOffsetX - gutterPositionOffset,
