...
package feathers.controls.text
{
	...
	public class TextFieldTextEditorViewPort extends TextFieldTextEditor implements ITextEditorViewPort
	{
		...
		override protected function measure(result:Point = null):Point
		{
			...
			{
				newWidth =
						this.measureTextField.width + this._paddingLeft + this._paddingRight - gutterDimensionsOffset;
			...
			var newHeight:Number = this.measureTextField.height + this._paddingTop + this._paddingBottom
								   - gutterDimensionsOffset;
			if (this._useGutter)
			{
				newHeight += 4;
			}
		...
		override protected function refreshTextFieldSize():void
		{
			...
			this.textField.scrollV = Math.round(
					1 + ((this.textField.maxScrollV - 1) * (this._verticalScrollPosition
		...
		override protected function transformTextField():void
		{
			...
			this.textField.x = offsetX + Math.round(
							starlingViewPort.x + (HELPER_POINT.x * scaleFactor) - gutterPositionOffset * scaleX
					);
			this.textField.y = offsetY + Math.round(
							starlingViewPort.y + (HELPER_POINT.y * scaleFactor) - gutterPositionOffset * scaleY
					);
		...
		override protected function positionSnapshot():void
		{
			...
			this.textSnapshot.y =
					this._paddingTop + this._verticalScrollPosition + Math.round(HELPER_MATRIX.ty) - HELPER_MATRIX.ty;
		}
		...
		protected function textField_scrollHandler(event:Event):void
		{
			...
			{
				var calculatedVerticalScrollPosition:Number = scroller.maxVerticalScrollPosition * (scrollV - 1)
															  / (this.textField.maxScrollV - 1);
