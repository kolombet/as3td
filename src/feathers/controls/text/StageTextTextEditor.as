	 ...
	 */ public class StageTextTextEditor extends FeathersControl implements IMultilineTextEditor
	{
		...
		override public function render(support:RenderSupport, parentAlpha:Number):void
		{
			if (this.textSnapshot && this._updateSnapshotOnScaleChange)
			{
				...
				if (matrixToScaleX(HELPER_MATRIX) != this._lastGlobalScaleX || matrixToScaleY(HELPER_MATRIX)
																			   != this._lastGlobalScaleY)
				{
				...
				if (position)
				{
					...
					else
					{
						this._pendingSelectionBeginIndex =
								this._measureTextField.getCharIndexAtPoint(positionX, positionY);
						if (this._pendingSelectionBeginIndex < 0)
						{
							if (this._multiline)
							{
							...
							{
								this._pendingSelectionBeginIndex = this._measureTextField.getCharIndexAtPoint(
										positionX,
		...
		protected function layout(sizeInvalid:Boolean):void
		{
			...
			if (sizeInvalid || stylesInvalid || skinInvalid || stateInvalid)
			{
				...
				this._needsNewTexture =
						this._needsNewTexture || !this.textSnapshot || textureRoot.scale != Starling.contentScaleFactor
						|| viewPort.width != textureRoot.width || viewPort.height != textureRoot.height;
			}
			
			if (!this._stageTextHasFocus && (stateInvalid || stylesInvalid || dataInvalid || sizeInvalid
											 || this._needsNewTexture))
			{
		...
		protected function doPendingActions():void
		{
			...
			if (this._pendingSelectionBeginIndex >= 0)
			{
				...
				var endIndex:int = (this._pendingSelectionEndIndex < 0) ? this._pendingSelectionBeginIndex :
								   this._pendingSelectionEndIndex;
				this._pendingSelectionBeginIndex = -1;
				this._pendingSelectionEndIndex = -1;
				if (this.stageText.selectionAnchorIndex != startIndex || this.stageText.selectionActiveIndex
																		 != endIndex)
				{
		...
		protected function refreshSnapshot():void
		{
			...
			try
			{
				var bitmapData:BitmapData = new BitmapData(
						viewPort.width * nativeScaleFactor,
						viewPort.height * nativeScaleFactor,
						true,
						0x00ff00ff
				);
			...
			if (!this.textSnapshot || this._needsNewTexture)
			{
				...
				newTexture = Texture.empty(
						bitmapData.width / scaleFactor, bitmapData.height / scaleFactor, true, false, false, scaleFactor
				);
		...
		protected function refreshViewPortAndFontSize():void
		{
			...
			if (this.is3D)
			{
				...
				HELPER_POINT3D = MatrixUtil.transformCoords3D(
						HELPER_MATRIX3D,
						-desktopGutterPositionOffset,
						-desktopGutterPositionOffset,
						0,
						HELPER_POINT3D
				);
			...
			var viewPortWidth:Number = Math.round(
					(this.actualWidth + desktopGutterDimensionsOffset) * scaleFactor * globalScaleX
			);
			if (viewPortWidth < 1 || viewPortWidth !== viewPortWidth) //isNaN
			{
				viewPortWidth = 1;
			}
			var viewPortHeight:Number = Math.round(
					(this.actualHeight + desktopGutterDimensionsOffset) * scaleFactor * globalScaleY
			);
		...
		protected function disposeStageText():void
		{
			...
			this.stageText.removeEventListener(
					...
					stageText_softKeyboardActivateHandler
			);
			this.stageText.removeEventListener(
					SoftKeyboardEvent.SOFT_KEYBOARD_DEACTIVATE,
					stageText_softKeyboardDeactivateHandler
			);
