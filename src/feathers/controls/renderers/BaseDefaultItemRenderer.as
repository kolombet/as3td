	...
	public class BaseDefaultItemRenderer extends ToggleButton implements IFocusContainer
	{
		...
		override public function dispose():void
		{
			...
			if (this._stateDelayTimer)
			{
				...
				this._stateDelayTimer.removeEventListener(
						TimerEvent.TIMER_COMPLETE,
						stateDelayTimer_timerCompleteHandler
				);
		...
		{
			if (this._isEnabled && !this._isToggle && (!this.isSelectableWithoutToggle || (this._itemHasSelectable
																						   && !this.itemToSelectable(this._data))))
			{
				value = STATE_UP;
			}
			if (this._useStateDelayTimer)
			{
				...
				if (value == Button.STATE_DOWN)
				{
					...
					else
					{
						...
						this._stateDelayTimer.addEventListener(
								TimerEvent.TIMER_COMPLETE,
								stateDelayTimer_timerCompleteHandler
						);
					}
		...
		protected function addAccessoryWidth(width:Number):Number
		{
			...
			if (this._accessoryPosition == ACCESSORY_POSITION_LEFT || this._accessoryPosition
																	  == ACCESSORY_POSITION_RIGHT)
			{
				if (hasPreviousItem)
				{
		...
		protected function addAccessoryHeight(height:Number):Number
		{
			...
			if (this._accessoryPosition == ACCESSORY_POSITION_TOP || this._accessoryPosition
																	 == ACCESSORY_POSITION_BOTTOM)
			{
				if (hasPreviousItem)
				{
					var adjustedAccessoryGap:Number = this._accessoryGap;
		...
		protected function replaceAccessory(newAccessory:DisplayObject):void
		{
			...
			if (this._itemHasAccessory && this.currentAccessory && this.currentAccessory != newAccessory
				&& this.currentAccessory.parent == this)
			{
				//the icon is created using the data provider, and it is not
				//created inside this class, so it is not our responsibility to
				//dispose the icon. if we dispose it, it may break something.
			...
			{
				this.accessoryLoader.delayTextureCreation =
						this._delayTextureCreationOnScroll && this._owner.isScrolling;
			}
		}
		...
		protected function refreshAccessory():void
		{
			...
			if (this._stateToAccessoryFunction != null)
			{
				this.currentAccessory =
						DisplayObject(this._stateToAccessoryFunction(this, this._currentState, oldAccessory));
			}
			else
			{
				this.currentAccessory = DisplayObject(
						this._accessorySelector.updateValue(
								this,
								this._currentState,
								this.currentAccessory
						)
			...
			if (this.currentAccessory != oldAccessory)
			{
				if (oldAccessory)
				{
					...
					{
						IFeathersControl(oldAccessory).removeEventListener(
								FeathersEventType.RESIZE,
								accessory_resizeHandler
						);
				...
				if (this.currentAccessory)
				{
					...
					if (this.currentAccessory is IFeathersControl)
					{
						IFeathersControl(this.currentAccessory).addEventListener(
								FeathersEventType.RESIZE,
								accessory_resizeHandler
						);
						IFeathersControl(this.currentAccessory).addEventListener(
								TouchEvent.TOUCH,
								accessory_touchHandler
						);
					}
				}
			}
		...
		protected function refreshIconLabel(label:String):void
		{
			if (!this.iconLabel)
			{
				var factory:Function = this._iconLabelFactory != null ? this._iconLabelFactory :
				...
				var iconLabelStyleName:String = this._customIconLabelStyleName != null ?
												this._customIconLabelStyleName : this.iconLabelStyleName;
		...
		protected function refreshAccessoryLabel(label:String):void
		{
			if (!this.accessoryLabel)
			{
				var factory:Function = this._accessoryLabelFactory != null ? this._accessoryLabelFactory :
									   FeathersControl.defaultTextRendererFactory;
				...
				var accessoryLabelStyleName:String = this._customAccessoryLabelStyleName != null ?
													 this._customAccessoryLabelStyleName : this.accessoryLabelStyleName;
		...
		override protected function layoutContent():void
		{
			...
			var accessoryIsInLayout:Boolean = this.currentAccessory && this._accessoryPosition
																	   != ACCESSORY_POSITION_MANUAL;
			...
			if (this._label && this.labelTextRenderer && iconIsInLayout && accessoryIsInLayout)
			{
				...
				if (this._layoutOrder == LAYOUT_ORDER_LABEL_ACCESSORY_ICON)
				{
					this.positionRelativeToOthers(
							this.currentAccessory,
							labelRenderer,
							null,
							this._accessoryPosition,
							accessoryGap,
							null,
							0
					);
		...
		override protected function refreshMaxLabelSize(forMeasurement:Boolean):void
		{
			...
			var hasIconToLeftOrRight:Boolean = this.currentIcon && (this._iconPosition == ICON_POSITION_LEFT
																	|| this._iconPosition == ICON_POSITION_LEFT_BASELINE
																	|| this._iconPosition == ICON_POSITION_RIGHT
																	|| this._iconPosition
																	   == ICON_POSITION_RIGHT_BASELINE);
			var hasIconToTopOrBottom:Boolean = this.currentIcon && (this._iconPosition == ICON_POSITION_TOP
																	|| this._iconPosition == ICON_POSITION_BOTTOM);
			var hasAccessoryToLeftOrRight:Boolean = this.currentAccessory && (this._accessoryPosition
																			  == ACCESSORY_POSITION_LEFT
																			  || this._accessoryPosition
																				 == ACCESSORY_POSITION_RIGHT);
			var hasAccessoryToTopOrBottom:Boolean = this.currentAccessory && (this._accessoryPosition
																			  == ACCESSORY_POSITION_TOP
																			  || this._accessoryPosition
																				 == ACCESSORY_POSITION_BOTTOM);
			
			if (this.accessoryLabel)
			{
				var iconAffectsAccessoryLabelMaxWidth:Boolean = hasIconToLeftOrRight && (hasAccessoryToLeftOrRight
																						 || this._layoutOrder
			...
			{
				var accessoryAffectsIconLabelMaxWidth:Boolean = hasAccessoryToLeftOrRight && (hasIconToLeftOrRight
																							  || this._layoutOrder
		...
		protected function positionRelativeToOthers(object:DisplayObject, relativeTo:DisplayObject, relativeTo2:DisplayObject, position:String, gap:Number, otherPosition:String, otherGap:Number):void
		{
			...
			var relativeToWidth:Number = relativeTo2 ?
										 (Math.max(relativeTo.x + relativeTo.width, relativeTo2.x + relativeTo2.width)
										  - relativeToX) : relativeTo.width;
			var relativeToHeight:Number = relativeTo2 ? (Math.max(
					relativeTo.y + relativeTo.height,
					relativeTo2.y + relativeTo2.height
			) - relativeToY) : relativeTo.height;
			...
			else if (position == ACCESSORY_POSITION_RIGHT)
			{
				...
				else
				{
					...
					if (relativeTo2)
					{
						newRelativeToX = Math.min(
								newRelativeToX,
								this.actualWidth - this._paddingRight - object.width - relativeToWidth - gap
						);
					}
			...
			else if (position == ACCESSORY_POSITION_BOTTOM)
			{
				...
				else
				{
					...
					if (relativeTo2)
					{
						newRelativeToY = Math.min(
								newRelativeToY,
								this.actualHeight - this._paddingBottom - object.height - relativeToHeight - gap
						);
					}
			...
			if (!relativeTo2 || otherGap != Number.POSITIVE_INFINITY || !(
				(position == ACCESSORY_POSITION_TOP && otherPosition == ACCESSORY_POSITION_TOP) || (position
																									== ACCESSORY_POSITION_RIGHT
																									&& otherPosition
																									   == ACCESSORY_POSITION_RIGHT)
				|| (position == ACCESSORY_POSITION_BOTTOM && otherPosition == ACCESSORY_POSITION_BOTTOM) || (position
																											 == ACCESSORY_POSITION_LEFT
																											 && otherPosition
																												== ACCESSORY_POSITION_LEFT)
					))
			...
			if (relativeTo2)
			{
				if (otherGap != Number.POSITIVE_INFINITY || !(
					(position == ACCESSORY_POSITION_LEFT && otherPosition == ACCESSORY_POSITION_RIGHT) || (position
																										   == ACCESSORY_POSITION_RIGHT
																										   && otherPosition
																											  == ACCESSORY_POSITION_LEFT)
					|| (position == ACCESSORY_POSITION_TOP && otherPosition == ACCESSORY_POSITION_BOTTOM) || (position
																											  == ACCESSORY_POSITION_BOTTOM
																											  && otherPosition
																												 == ACCESSORY_POSITION_TOP)
						))
				...
				if (gap == Number.POSITIVE_INFINITY && otherGap == Number.POSITIVE_INFINITY)
				{
					...
					{
						relativeTo.x = relativeTo2.x + Math.round(
										(object.x - relativeTo2.x + relativeTo2.width - relativeTo.width) / 2
								);
					}
					else if (position == ACCESSORY_POSITION_LEFT && otherPosition == ACCESSORY_POSITION_RIGHT)
					{
						relativeTo.x =
								object.x + Math.round((relativeTo2.x - object.x + object.width - relativeTo.width) / 2);
					}
					else if (position == ACCESSORY_POSITION_RIGHT && otherPosition == ACCESSORY_POSITION_RIGHT)
					{
						relativeTo2.x = relativeTo.x + Math.round(
										(object.x - relativeTo.x + relativeTo.width - relativeTo2.width) / 2
								);
					}
					else if (position == ACCESSORY_POSITION_LEFT && otherPosition == ACCESSORY_POSITION_LEFT)
					{
						relativeTo2.x =
								object.x + Math.round((relativeTo.x - object.x + object.width - relativeTo2.width) / 2);
					}
					else if (position == ACCESSORY_POSITION_BOTTOM && otherPosition == ACCESSORY_POSITION_TOP)
					{
						relativeTo.y = relativeTo2.y + Math.round(
										(object.y - relativeTo2.y + relativeTo2.height - relativeTo.height) / 2
								);
					}
					else if (position == ACCESSORY_POSITION_TOP && otherPosition == ACCESSORY_POSITION_BOTTOM)
					{
						relativeTo.y = object.y + Math.round(
										(relativeTo2.y - object.y + object.height - relativeTo.height) / 2
								);
					}
					else if (position == ACCESSORY_POSITION_BOTTOM && otherPosition == ACCESSORY_POSITION_BOTTOM)
					{
						relativeTo2.y = relativeTo.y + Math.round(
										(object.y - relativeTo.y + relativeTo.height - relativeTo2.height) / 2
								);
					}
					else if (position == ACCESSORY_POSITION_TOP && otherPosition == ACCESSORY_POSITION_TOP)
					{
						relativeTo2.y = object.y + Math.round(
										(relativeTo.y - object.y + object.height - relativeTo2.height) / 2
								);
					}
				}
			}
			...
			if (position == ACCESSORY_POSITION_LEFT || position == ACCESSORY_POSITION_RIGHT)
			{
				...
				{
					object.y = this._paddingTop + Math.round(
									(this.actualHeight - this._paddingTop - this._paddingBottom - object.height) / 2
							);
				}
			}
			else if (position == ACCESSORY_POSITION_TOP || position == ACCESSORY_POSITION_BOTTOM)
			{
				...
				{
					object.x = this._paddingLeft + Math.round(
									(this.actualWidth - this._paddingLeft - this._paddingRight - object.width) / 2
							);
				}
			}
		}
		...
		{
			if (this._isToggle || !this.isSelectableWithoutToggle || (this._itemHasSelectable
																	  && !this.itemToSelectable(this._data)))
			{
				return;
			}
		...
		{
			if (this.currentAccessory && !this._isSelectableOnAccessoryTouch && this.currentAccessory
																				!= this.accessoryLabel
				&& this.currentAccessory != this.accessoryLoader && this.touchPointID < 0)
			{
		...
		protected function accessory_touchHandler(event:TouchEvent):void
		{
			...
			if (!this._stopScrollingOnAccessoryTouch || this.currentAccessory === this.accessoryLabel
