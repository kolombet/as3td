...
package feathers.controls
{
	...
	public class Header extends FeathersControl
	{
		...
		override protected function draw():void
		{
			...
			if (textRendererInvalid || sizeInvalid || stylesInvalid || dataInvalid || leftContentInvalid
				|| rightContentInvalid || centerContentInvalid)
			{
				this.layoutTitle();
			}
		...
		protected function createTitle():void
		{
			...
			var factory:Function = this._titleFactory != null ? this._titleFactory :
			...
			var titleStyleName:String = this._customTitleStyleName != null ? this._customTitleStyleName :
										this.titleStyleName;
		...
		protected function calculateExtraOSStatusBarPadding():Number
		{
			...
			if (os.indexOf(IOS_NAME_PREFIX) != 0 || parseInt(os.substr(IOS_NAME_PREFIX.length, 1), 10)
													< STATUS_BAR_MIN_IOS_VERSION)
			{
				return 0;
			}
			...
			return IOS_STATUS_BAR_HEIGHT * Math.floor(DeviceCapabilities.dpi / IOS_DPI)
				   / Starling.current.contentScaleFactor;
		}
		...
		protected function layoutTitle():void
		{
			...
			var leftOffset:Number = (this._leftItems && this._leftItems.length > 0) ?
									(this.leftItemsWidth + calculatedTitleGap) : 0;
			var rightOffset:Number = (this._rightItems && this._rightItems.length > 0) ?
									 (this.rightItemsWidth + calculatedTitleGap) : 0;
			...
			else if (this._titleAlign == TITLE_ALIGN_PREFER_RIGHT && (!this._rightItems || this._rightItems.length
																						   == 0))
			{
			...
			else
			{
				...
				this.titleTextRenderer.validate();
				var idealTitlePosition:Number = this._paddingLeft + (actualWidthMinusPadding
																	 - this.titleTextRenderer.width) / 2;
				if (leftOffset > idealTitlePosition || (idealTitlePosition + this.titleTextRenderer.width)
													   > (this.actualWidth - rightOffset))
				{
					this.titleTextRenderer.x =
							leftOffset + (actualWidthMinusOffsets - this.titleTextRenderer.width) / 2;
				}
				else
				{
			...
			{
				this.titleTextRenderer.y = paddingTop + (this.actualHeight - paddingTop - this._paddingBottom
														 - this.titleTextRenderer.height) / 2;
			}
		}
