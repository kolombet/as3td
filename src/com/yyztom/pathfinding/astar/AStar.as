package com.yyztom.pathfinding.astar
{
	...
	public class AStar
	{
		...
		public function AStar(grid:Vector.<Vector.<AStarNodeVO>>)
		{
			...
			_grid = grid;
			_openHeap = new BinaryHeap(
					function (node:AStarNodeVO):Number
					{
						return node.f;
					}
			);
		...
		private function getNeighbors(grid:Vector.<Vector.<AStarNodeVO>>, node:AStarNodeVO):Vector.<AStarNodeVO>
		{
			var ret:Vector.<AStarNodeVO> = new Vector.<AStarNodeVO>(
						8,
						true
				), x:uint                = node.position.x, y:uint = node.position.y, gridWidth:uint = grid.length, gridHeight:uint = grid[x].length, id:uint;
