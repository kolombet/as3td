...
package starling.utils
{
     ...
     *  add assetManager directly (via the 'add...' methods) or asynchronously via a queue. This allows
     *  you to deal with assetManager in a unified way, no matter if they are loaded from a file, 
     ...
     *  </p>
     *  
     *  <p>For more information on how to add assetManager from different sources, read the documentation
     ...
     *
     *  <p>Loading of some assetManager may fail while the queue is being processed. In that case, the
     ...
     *  assetManager.enqueue(appDir.resolvePath("textures/32bit"));
     *  
     *  assetManager.textureFormat = Context3DTextureFormat.BGRA_PACKED;
     *  assetManager.enqueue(appDir.resolvePath("textures/16bit"));
     *  
     *  assetManager.loadQueue(...);</listing>
     */
    public class AssetManager extends EventDispatcher
    {
         ...
         *
         *  <p>Beware that all references to the assetManager will remain intact, even though the assetManager
        ...
        /** Removes assetManager of all types (disposing them along the way), empties the queue and
         *  aborts any pending load operations. */
        public function purge():void
        {
            log("Purging all assetManager, emptying queue");
        ...
        /** Enqueues one or more raw assetManager; they will only be available after successfully 
         ...
         *    <li>Classes that contain <code>static</code> embedded assetManager.</li>
         ...
         *  accessible under the name "image". When enqueuing embedded assetManager via a class, 
        ...
        public function enqueue(...rawAssets):void
        {
            for each (var rawAsset:Object in rawAssets)
            {
                ...
                else if (rawAsset is Class)
                {
                    ...
                    if (mVerbose)
                        log("Looking for static embedded assetManager in '" + 
        ...
        /** Loads all enqueued assetManager asynchronously. The 'onProgress' function will be called
        ...
        public function loadQueue(onProgress:Function):void
        {
            ...
            if (mStarling == null || mStarling.context == null)
                throw new Error("The Starling instance needs to be ready before assetManager can be loaded.");
                                         ...
                                         onProgress:Function, onComplete:Function):void
        {
            ...
            function process(asset:Object):void
            {
                ...
                {
                    log("Context lost while processing assetManager, retrying ...");
         ...
         *  be called for embedded assetManager.
         *
        ...
        /** This method is called during loading of assetManager when 'verbose' is activated. Per
        ...
        /** Returns the number of raw assetManager that have been enqueued, but not yet loaded. */
        public function get numQueuedAssets():int { return mQueue.length; }
        
        /** When activated, the class will trace information about added/enqueued assetManager.
         *  @default true */
